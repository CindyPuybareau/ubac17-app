...
export type BenevoleNotification = {
  id: string;
  title: string;
  body: string;
  createdAt: string;
...
// Même comportement que ChildNotificationBell côté Espace Enfant (voir
// enfant/view/child-notification-bell.tsx), avec les mêmes deux
// différences imposées par l'architecture bénévole (pas d'auth.uid(), pas
// de session Supabase) : liste initiale reçue en props (lue en
// service_role par page.tsx), écriture toujours via une route API dédiée
// (/api/benevole-notifications/*) qui revérifie le cookie de session.
export default function BenevoleNotificationBell({
  initialNotifications,
  initialEnabled,
}: {
  initialNotifications: BenevoleNotification[];
  ...
  async function toggleOpen() {
    ...
    if (next && unreadCount > 0 && enabled) {
      setMarking(true);
      try {
        const res = await fetch("/api/benevole-notifications/read-all", {
          method: "POST",
        ...
        // Silencieux : le badge se remettra à jour au prochain chargement.
      } finally {
        setMarking(false);
      }
    }
  ...
  async function togglePreference() {
    ...
    setSavingPref(true);
    try {
      const res = await fetch("/api/benevole-notifications/preference", {
        method: "POST",
        credentials: "same-origin",
      ...
      {open && (
        <div className="fixed inset-x-4 top-16 z-30 overflow-hidden rounded-2xl border border-zinc-100 bg-white shadow-xl sm:absolute sm:inset-x-auto sm:right-0 sm:top-full sm:mt-2 sm:w-80">
          ...
          <div className="max-h-80 overflow-y-auto">
            {!enabled ? (
              <p className="px-4 py-6 text-center text-sm text-zinc-400">Notifications désactivées.</p>
