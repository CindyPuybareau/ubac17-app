...
export default function PlayerYearBadge({
  ...
  if (status.kind === "ROOKIE") {
    return (
      <span className="inline-flex w-fit items-center whitespace-nowrap rounded-full bg-ubac-yellow/20 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-ubac-yellow-dark">
        Rookie
      </span>
    );
  }
  ...
  if (status.kind === "SPARRING") {
    return (
      <span className="inline-flex w-fit items-center whitespace-nowrap rounded-full bg-purple-100 px-2 py-0.5 text-[10px] font-semibold text-purple-700">
        Sparring Partner
      </span>
    );
  }
